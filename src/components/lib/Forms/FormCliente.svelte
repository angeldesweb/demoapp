<script>
    ...
    import { clientSchema } from '../../../tools/validations/clients';
    import { createClient } from '../../../tools/api/clients';
    ...
    const handleInvalid = async (values) => {
        try {
            await clientSchema.validate(values);
            return true;
        } catch (error) {
    ...
    const handleSubmit = async (e) => {
        ...
        const valid = await clientSchema.isValid(values);
...
<div class="card">
    <form on:submit|preventDefault={handleSubmit}>
        ...
        <input type="text" name="name" id="name" placeholder="Nombre del cliente" required>
